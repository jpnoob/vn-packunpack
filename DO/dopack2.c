...
/* same as dopack but sorts filenames in the same order as the game
   ('.' < '_' < the rest)
*/
...
int equalname(char *a,char *b) {
	int i;
	for(i=0;i<12 && a[i] && b[i];i++) if(toupper(a[i])!=toupper(b[i])) return 0;
	return i==12 || a[i]==b[i];
}
...
void pack(char *dest) {
	...
	for(int i=0;i<12 && dest[i];i++) buf[i+0x10]=tolower(dest[i]);
	/* write offset to dir list at 0x1c */
	writeuint32(buf+0x1c,0x20);

	/* preliminary pass to gather filenames */
#ifdef _WIN32
	findfirst("*",&t);
#else
	findfirst(".",&t);
#endif
	unsigned long long atdir=0x20;
	/* write contents to 16-aligned start address */
	resstart=(resstart+15)/16*16;
	do {
		if(t.dir==0) {
			/* write filename (max 12 chars) */
			for(int i=0;i<12 && t.s[i];i++) buf[atdir+i]=toupper(t.s[i]);
	...
	findclose(&t);
	/* sort filenames */
	qsort(buf+0x20,numfiles,0x14,compdir);

	/* read all files and gradually build archive file in memory */
	/* write contents to 16-aligned start address */
	resstart=(resstart+15)/16*16;
	atdir=0x20;
	for(unsigned int files=0;files<numfiles;files++) {
		char s[13];
		for(int i=0;i<12;i++) s[i]=buf[atdir+i];
		s[12]=0;
		unsigned len=readuint32(buf+atdir+0x10);
		...
		if(fread(buf+resstart,1,len,f)!=len) { puts("file read error"); exit(1); }
		if(fclose(f)) { puts("error closing file for reading"); exit(1); }
		/* set start of next file */
		resstart=(resstart+len+15)/16*16;
		/* advance dir pointer */
		atdir+=20;
	}
